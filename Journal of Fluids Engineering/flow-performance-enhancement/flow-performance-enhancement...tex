...
\begin{document}
	...
	\begin{enumerate}
		\item The captions for figures and tables should be clear and provide enough information so the reader is not required to refer back to the main body of the text.
		\item There is a lot of assumed or implied knowledge when reading this manuscript. I have made specific comments in the respective sections.
		\item The authors appear to make several interpretations that are stated casually when the available evidence presented in the study appears to be lacking. These statements should be qualified or evidence provided to support the claims. I have identified some specific cases in the following specific comments.
	\end{enumerate}
	...
	\begin{enumerate}
		\item The authors should check Eq.~(4) against Oz and Kara [35]. In particular, $\overline{u}_j$ may indicate a Reynolds mean where the Favre-averaged velocity $\widetilde{u}_j$ is intended. The sign of the conductive heat-flux term should also be checked against the cited formulation. If the viscosity inconsistency in Eq.~(2) is confirmed, the same issue propagates into Eq.~(4) and should also be checked.
		
		\item Not all terms in Eq.~(4) are defined when they are introduced. Some symbols also overlap with later notation. Since the authors state that internal energy and enthalpy are Favre averaged, the definitions of $\widetilde{e}$ and $\widetilde{h}$ should explicitly identify these as the Favre-averaged specific internal energy and specific enthalpy.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item The axes in Figure~4 should be clearly defined, including the coordinate directions and units. More generally, figure and table captions should provide sufficient information for the corresponding figure or table to be interpreted without relying excessively on the main text.
		
		\item The authors should consider including the oscillation frequency associated with each configuration shown in Figure~4, either in the figure or its caption. This would help relate the instantaneous flow fields and phase angles to the different oscillation periods of each configuration.
